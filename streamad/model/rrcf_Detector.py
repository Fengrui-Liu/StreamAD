...
class RCForest(BaseDetector):
    def __init__(self, num_trees=40, shingle_size=4, tree_size=256):
        ...
        Args:
            ...
            shingle_size (int, optional): sliding window when fit stream data. that means the
    ...
    def fit(self, X: np.ndarray) -> None:
        ...
        # of shingle_size.
        # Ps. the function shingle needs a iterable object.
        if not isinstance(X, Iterable):
            self.shingle_list.append(X)
            if len(self.shingle_list) > self.shingle_size:
                self.shingle_list.pop(0)
            if len(self.shingle_list) == self.shingle_size:
                points = shingle(self.shingle_list, size=self.shingle_size)
            else:
                return self
        # when stream data come as a whole, directly deliver to function shingle.
        else:
            points = shingle(X, size=self.shingle_size)
        ...
        for index, point in enumerate(points):
            ...
            for tree in self.forest:
                ...
                if not self.index in self.avg_codisp:
                    self.avg_codisp[self.index] = 0
                self.avg_codisp[self.index] += tree.codisp(self.index) / self.num_trees
            self.stat.update(self.avg_codisp[self.index])
            self.index += 1
    ...
    def score(self, X: np.ndarray, all=False) -> float:
        ...
        # if length of anomaly != index, that means there are new stream data arrived since last call
        # function score. then we get new avg and std, so update all anomaly scores.
        if len(self.anomaly) != self.index:
            self.anomaly = list(range(self.index))
        # tmp is defined as how many times it exceed std. then it multiply by itself and divided by 20,
        # then put it into tanh to get anomaly probability.
        for index in range(self.index):
            if self.stat.get_std() == 0:
                tmp = 0
            else:
                tmp = (
                    self.avg_codisp[index] - self.stat.get_mean()
                ) / self.stat.get_std()
            tmp = tmp * tmp / 20
            self.anomaly[index] = math.tanh(tmp) * math.tanh(tmp)
