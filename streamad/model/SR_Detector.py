...
class SRDetector(BaseDetector):
    def __init__(
        ...
        self.conv_local = (
            np.ones((1, window_local + 1)).reshape(-1,) / window_local
        )
        self.conv_local[0] = 0
    ...
    def fit(self, X: np.ndarray) -> None:
        self.data_record.append(X)
        if len(self.data_record) > 10000:
            self.data_record.pop(0)
        return

    def score(self, y: np.ndarray, all=False):
        if len(self.data_record) == 0:
            return None
        X = np.array(self.data_record)
        ...
        iscore = self.outlier_score(X, t)

        # values above threshold are outliers
        self.threshold = np.percentile(iscore, 90)
    ...
    def outlier_score(self, X: np.ndarray, t: np.ndarray = None) -> np.ndarray:
        """
        Compute outlier scores.
        Parameters
        ----------
